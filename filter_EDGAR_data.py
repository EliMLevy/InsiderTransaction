...
def filtered_transactions(path_to_data):
    ...
    print(f"Number transactions before filtering: {len(transactions)}")

    # We are only interested in:
    # - form type 4
    # - acquisitions
    # - direct ownership
    # - transactions that have a price and number of shares listed
    # - transactions where the shares cost > $0
    ...
    print(f"Number transactions after filter 1: {len(transactions)}")



    transactions = transactions.merge(owners, how='left', on='ACCESSION_NUMBER')
    transactions = transactions.merge(submissions, how='left', on='ACCESSION_NUMBER')
    ...
    print(f"Number transactions after filter 2: {len(transactions)}")

    def convert_date(date_string):
        return str(datetime.strptime(date_string, "%d-%b-%Y"))

    transactions["datetime"] = transactions['FILING_DATE'].apply(convert_date)
    print(f"Number transactions after filter 3: {len(transactions)}")

    transactions = transactions[transactions["RPTOWNER_RELATIONSHIP"].str.contains('.*(officer|director).*', case=False, regex=True)]
    print(f"Number transactions after filter 4: {len(transactions)}")
...
def main():
    result = filtered_transactions("./data/small_ds/")
    print(result)
